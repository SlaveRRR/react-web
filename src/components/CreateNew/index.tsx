  ...
  return (
    ...
    <Form onSubmit={HookHandleSubmit(handleSubmit)}>
      ...
      >
        <Input
          ...
          {...register("username", {
            ...
            validate: (value) =>
              value.length >= 5 ||
              `Required one more ${5 - value.length} symbols`,
          })}
        />
        <div>
          {errors?.username && (
      ...
      <Item<IForm> label="Comment" name="comment" rules={[{ required: true }]}>
        <Input
          ...
          {...register("comment", {
            ...
            validate: (value) =>
              value.length >= 10 ||
              `Required one more ${10 - value.length} symbols`,
          })}
        />
        <div>
          {errors?.comment && (
            <Text type="danger">{errors?.comment.message}</Text>
          )}
        </div>
      </Item>
      ...
      <Item<IForm> label="Picture" name="picture" rules={[{ required: true }]}>
        ...
        <div>
          {errors?.picture && (
            <Text type="danger">{errors?.picture.message}</Text>
          )}
        </div>
      </Item>
